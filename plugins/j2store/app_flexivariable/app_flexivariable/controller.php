    ...
    public function addFlexiVariant(){
        ...
        if(isset($post['varient_combin']) && !empty($post['varient_combin']) && isset($post['flexi_product_id']) && !empty($post['flexi_product_id'])){
            ...
            foreach ($post['varient_combin'] as $variant_key => $variant_value){
                //get Product Option 
                $product_option = F0FModel::getTmpInstance('ProductOption' ,'J2StoreModel')->getItem($variant_key);
            ...
            if(!empty($product_optionvalue_ids)){
                ...
                if(isset($variantChild->j2store_variant_id) && $variantChild->j2store_variant_id){
                    ...
                    $product_quantity = F0FTable::getAnInstance('Productquantity','J2StoreTable')->getClone();
        ...
        $json = array();
        $json['html'] = $this->getVariantHtml($post);
        ...
        try {
            ...
            foreach ($productPrices as $price) {
                if($price->variant_id == $id) {
                    $product_price_table = F0FTable::getAnInstance('ProductPrice', 'J2StoreTable')->getClone();
            ...
            $varaint_table = F0FTable::getAnInstance('Variant', 'J2StoreTable')->getClone();
