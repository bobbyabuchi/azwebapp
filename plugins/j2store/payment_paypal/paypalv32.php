...
class plgJ2StorePayment_paypal extends J2StorePaymentPlugin
{
    ...
    function _prePaymentForSubscriptionProduct( $data )
    {
        ...
        $returnUrl = $rootURL.JRoute::_("index.php?option=com_j2store&view=checkout&task=confirmPayment&orderpayment_type=".$this->_element."&paction=display");
        $cancelUrl = $rootURL.JRoute::_("index.php?option=com_j2store&view=checkout&task=confirmPayment&orderpayment_type=".$this->_element."&paction=cancel");;
...
//			$postFields['L_PAYMENTREQUEST_0_NUMBER'.$i] = urlencode($i);
                $postFieldsItems['L_PAYMENTREQUEST_0_AMT'.$i] = urlencode($tax);
...
//			$postFieldsItems['L_PAYMENTREQUEST_0_NUMBER'.$i] = urlencode($i);
            $postFieldsItems['L_PAYMENTREQUEST_0_AMT'.$i] = urlencode(- $discount);
            $postFieldsItems['L_PAYMENTREQUEST_0_QTY'.$i] = urlencode(1);
            $i++;
        ...
        if($orderTotalValue != $amount){
            ...
            $postFields['L_PAYMENTREQUEST_0_AMT0'] = urlencode($amount);
    ...
    function _prePayment( $data )
    {
                ...
                foreach($order_items as $order_item) {
                    if($order_item->product_type == 'subscriptionproduct' || $order_item->product_type == 'variablesubscriptionproduct'){
                        ...
                        $variant_table->load(array(
                            'j2store_variant_id' => $order_item->variant_id
                        ));
        ...
        $vars->return_url = $rootURL.JRoute::_($return_url);//JRoute::_("index.php?option=com_j2store&view=checkout&task=confirmPayment&orderpayment_type=".$this->_element."&paction=display");
    ...
    protected function byRefference($subscription, $order){
        ...
        if(isset($subscription->meta['billing_agreement_id']['metavalue']) && $subscription->meta['billing_agreement_id']['metavalue'] != ''){
            ...
            } else if(isset($paymentResponse['ACK']) && $paymentResponse['ACK'] == 'Failure' && $paymentResponse['L_ERRORCODE0'] == '10412'){
...
//						'order_id' => $order_id
     ...
     * */
    protected function updateBillingAgreementId($order_id, $billingAgreementID = ''){
    ...
    public function onJ2StoreProcessCron($task, $timeperiod=''){
        ...
        if($task == 'paypal_tls_check'){
            ...
            $targetURL->setVar('VERSION', '106.0');
            $targetURL->setVar('TRANSACTIONCLASS', 'All');
...
class PaypalExpressCheckoutForJ2StoreSubscriptionProduct{
     ...
     *
     * @param array $data
     *        	Array of product params
     * @return string The response returned by Paypal
     */
