...
class SppagebuilderAddonDeals_slideshow extends SppagebuilderAddons {
			...
			foreach ($settings->slideshow_items as $item_key => $item_value) {
				...
				$output .='<div id="'.$uniqid.'" class="sp-item '. (($item_key == 0) ? ' active' : '') .' '.($content_vertical_alignment ? 'slider-content-vercally-center' : '').'">';
					if($content_container_option==='bootstrap'){
						...
						$output .= '<div class="sppb-inner-wrapper">';
					...
					$module_id = (isset($item_value->module_id) && $item_value->module_id) ? $item_value->module_id : 0;
						...
						if($content_container_option==='bootstrap'){
							$output .='</div>';//.sppb-col-sm-12
								...
								$modules = self::getModules($module_id);
								$attribs['style'] = 'sp_xhtml';
								if($modules){
									$output .= '<div class="slider-mod-wrap right-content">';
	...
	// Get module by ID
	private static function getModules($module_id = 0) {
