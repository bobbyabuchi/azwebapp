...
<div class="options">
	...
	<br />
        ...
        <?php if ($option['type'] == 'radio') {
			...
			$option_class = '';
			if (strpos(strtolower($option['option_name']), 'color') !== false) {
				$option_class = 'arino-option-color';
			} elseif (strpos(strtolower($option['option_name']), 'size') !== false) {
				$option_class = 'arino-option-size';
			} else {
				$option_class = ( isset($option['option_name']) && $option['option_name'] ) ? str_replace(' ', '-', $option['option_name']) : '' ;
			}

		?>
          <!-- radio -->
	<div id="option-<?php echo $option['productoption_id']; ?>"
		class="option <?php echo $option_class; ?>">
          <?php if ($option['required']) { ?>
          <span class="required">*</span>
          <?php } ?>
          ...
          <label for="option-value-<?php echo $option_value['product_optionvalue_id']; ?>">
			<span><?php echo stripslashes(JText::_($option_value['optionvalue_name'])); ?></span>
            <?php if ($option_value['product_optionvalue_price'] > 0 && $this->params->get('product_option_price', 1)) { ?>
	         	(
	         	<?php if($this->params->get('product_option_price_prefix', 1)): ?>
            		<?php echo $option_value['product_optionvalue_prefix']; ?>
