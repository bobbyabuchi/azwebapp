	...
	<?php if( count($list) > 0 ):?>
		<?php foreach ($list as $product_id => $product) : ?>
			...
			<div class="span<?php echo round((12 / $total_cols));?>">
				<div itemprop="itemListElement" itemscope="" itemtype="http://schema.org/Product" class="j2store product-<?php echo $product->j2store_product_id; ?> j2store-module-product j2store-product">
					...
					<div class="content-wrap">
						...
						<!-- Start Deals -->
						<?php if( ($product->show_price && $product->show_special_price) && $product->pricing->base_price != $product->pricing->price && $product->pricing->is_discount_pricing_available) { 
								$today    			= JHtml::date(JFactory::getDate(), 'Y-m-d');
								$deal_date 			= JHtml::date($product->pricing->data->date_to, 'Y-m-d');
								$deal_date_explode 	= explode('-', $deal_date);
								// if deals end date not less than today
								if ($deal_date > $today) {
							?>
							<div id="j2-deals-countdown<?php echo $product->j2store_product_id; ?>" class="clearfix"></div>
							<script type="text/javascript">
								jQuery(function($) {
									$('#j2-deals-countdown<?php echo $product->j2store_product_id; ?>').countdown('<?php echo trim($deal_date_explode[0]); ?>/<?php echo trim($deal_date_explode[1]); ?>/<?php echo trim($deal_date_explode[2]); ?>', function(event) {
										$(this).html(event.strftime('<div class="j2-deal-days"><span class="j2-deal-number">%-D</span><span class="j2-deal-string">%!D:<?php echo JText::_("HELIX_ULTIMATE_DAY"); ?>,<?php echo JText::_("HELIX_ULTIMATE_DAYS"); ?>;</span></div><div class="j2-deal-hours"><span class="j2-deal-number">%H</span><span class="j2-deal-string">%!H:<?php echo JText::_("HELIX_ULTIMATE_HOUR"); ?>,<?php echo JText::_("HELIX_ULTIMATE_HOURS"); ?>;</span></div><div class="j2-deal-minutes"><span class="j2-deal-number">%M</span><span class="j2-deal-string">%!M:<?php echo JText::_("HELIX_ULTIMATE_MINUTE"); ?>,<?php echo JText::_("HELIX_ULTIMATE_MINUTES"); ?>;</span></div><div class="j2-deal-seconds"><span class="j2-deal-number">%S</span><span class="j2-deal-string">%!S:<?php echo JText::_("HELIX_ULTIMATE_SECOND"); ?>,<?php echo JText::_("HELIX_ULTIMATE_SECONDS"); ?>;</span></div>'));
									});
								});
							</script>
						<?php } } ?>
								...
								<div class="product_cart_block">
									...
									<?php if($j2params->get('catalog_mode', 0) == 0 ) : ?>
										<form action="<?php echo $product->cart_form_action; ?>"
											...
											<?php if($cart_type == 1):?>
												<?php
												if ( $product->product_type=='simple' || $product->product_type=='downloadable' ):
											...
											<?php elseif( $product->product_type=='configurable' || ($cart_type == 2 && count($product->options)) || ($cart_type == 3)):?>
												<a href="<?php echo $product->module_display_link; ?>" data-toggle="tooltip" data-placement="top" title="<?php echo JText::_('J2STORE_VIEW_PRODUCT_DETAILS'); ?>" class="j2store-button-cart view-details-btn <?php echo $params->get('choosebtn_class', 'sppb-btn sppb-btn-link'); ?>"><i class="fa fa-link"></i></a>
											...
											<?php if ($product->display_cart_block): ?>
												...
												<div id="add-to-cart-<?php echo $product->j2store_product_id; ?>" class="j2store-add-to-cart">
													<img src="<?php echo JURI::root(true);?>/images/demo/cart-icon.svg" alt="cart">
													...
													<input
														data-toggle="tooltip" data-placement="top" title="<?php echo $product->cart_button_text; ?>"
														...
														type="submit"
														class="j2store-cart-button <?php echo $params->get('addtocart_button_class', 'sppb-btn sppb-btn-link');?>"
													/>
									...
									<?php if($product->show_quickview):?>
										...
										<?php else:?>
											<?php $product->product_quickview_link = JRoute::_( $product->module_display_link ).'?tmpl=component';?>
										<?php endif;?>
																			<a data-fancybox data-type="ajax" data-src="<?php echo $product->product_quickview_link; ?>" href="javascript:;" class="j2store-product-quickview-modal" data-toggle="tooltip" data-placement="top" title="<?php echo JText::_('J2STORE_PRODUCT_QUICKVIEW');?>"><i class="fa fa-eye"></i></a>
									<?php endif;?>
								</div>
							</div>
