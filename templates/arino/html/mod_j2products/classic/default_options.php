...
<div class="options">
	...
	<div id="j2option-<?php echo $option['productoption_id']; ?>"
		...
		<select
            ...
            <?php foreach ($option['optionvalue'] as $option_value) {?>
            	<?php $checked = '';if ($option_value['product_optionvalue_default']) {
    $checked = 'selected="selected"';
}
    ?>

            <option <?php echo $checked; ?>
				value="<?php echo $option_value['product_optionvalue_id']; ?>"><?php echo stripslashes(htmlspecialchars(JText::_($option_value['optionvalue_name']))); ?>
            <?php if ($option_value['product_optionvalue_price'] > 0 && $j2params->get('product_option_price', 1)) {?>
            (
            <?php if ($j2params->get('product_option_price_prefix', 1)): ?>
            	<?php echo $option_value['product_optionvalue_prefix']; ?>
            <?php endif;?>
            <?php echo $product_helper->displayPrice($option_value['product_optionvalue_price'], $product, $j2params); ?>
            )
            <?php }?>
            </option>
            <?php }?>
    ...
    ?>
          <label for="option-value-<?php echo $option_value['product_optionvalue_id']; ?>">
					<img
				class="optionvalue-img optionvalue-image-<?php echo $option_value['product_optionvalue_id']; ?>"
	...
	<div id="j2option-<?php echo $option['productoption_id']; ?>"
		...
		<label
			...
			<b><?php echo JText::_($option['option_name']); ?>:</b>
			<?php echo stripslashes(htmlspecialchars(JText::_($option_value['optionvalue_name']))); ?>
            <?php if ($option_value['product_optionvalue_price'] > 0 && $j2params->get('product_option_price', 1)) {?>
               (
               <?php if ($j2params->get('product_option_price_prefix', 1)): ?>
	...
	<br />
		...
		<script type="text/javascript">
			(function($) {
				...
				$('#j2option-'+po_id+' input:checkbox').bind("click",function(){
				    var product_id = '<?php echo $product_id ?>';
					doAjaxPrice(product_id, '#j2option-'+po_id+' input:checkbox');
				});
			})(j2store.jQuery);
		</script>
...
$('#product-option-<?php echo $option['productoption_id']; ?>').on('click', function() {
	...
	timer = setInterval(function() {
		if ($('#form-upload input[name=\'file\']').val() != '') {
			clearInterval(timer);
			$.ajax({
				url: 'index.php?option=com_j2store&view=carts&task=upload&product_id='+<?php echo $product->j2store_product_id; ?>,
				type: 'post',
				dataType: 'json',
