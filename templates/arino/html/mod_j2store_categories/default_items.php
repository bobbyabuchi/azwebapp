...
if ($category_display_view === "grid_view") {
    ...
    ?>
    <ul class="j2store-category-items j2store-categories-module<?php echo $moduleclass_sfx; ?>">
        <?php
        foreach ($list as $item) {

            $active_class = '';
            if( ($_SERVER['PHP_SELF'] == JRoute::_(ContentHelperRoute::getCategoryRoute($item->id))) ||  $active_id == $item->id) {
                $active_class = 'active';
            }

            ?>
            <li class="j2store-category-item <?php echo $active_class; ?>">
                <?php
                $category_params = json_decode($item->params);
                $image = $category_params->image;
                $image_alt = $category_params->image_alt;
